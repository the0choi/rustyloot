...
const BadgeCard = ({ badgeImage, badgeStyle, badgeName, textColor, backgroundColor, icon, textContent, semicircleGradient, circleGradient, bottomGradient, current }) => {
  return (
    <div className="sm:w-[130px] xl:w-[170px] flex flex-col items-start">
      <div className={`relative sm:h-[150px] xl:h-[215px] flex flex-col justify-end items-center xl:gap-[20px] self-stretch xl:pt-[34px] rounded-[6px] ${backgroundColor}`}>
        {current && (
        ...
        <div className="sm:w-[130px] xl:w-[170px] sm:h-[42px] xl:h-[68.331px] absolute left-0 top-0 sm:translate-y-[54px] xl:translate-y-[73px]">
          {semicircleGradient}
        </div>
        <div className="relative w-[54px] xl:w-[86px] h-[54px] xl:h-[86px] shrink-0">
          {circleGradient}
          <img src={badgeImage} className={`shrink-0 z-10 absolute inset-0 ${badgeStyle}`} />
        </div>

        <div className={`h-[74.7px] pb-[20px] xl:pb-[34px] flex flex-col justify-end items-center gap-[2px] xl:gap-[4px] shrink-0 self-stretch ${bottomGradient}`}>
          <span className="text-[#FFF] text-[14px] font-bold leading-[18px] tracking-[0.1px] z-10">
            {badgeName}
          </span>

          <div className="w-[73.7px] h-[18.7px] flex justify-center items-center gap-[4px] shrink-0">
            <div className="w-[12px] h-[12px] shrink-0">
              {icon}
            </div>
            <span className="text-[12px] font-bold leading-[15px] tracking-[0.1px]" style={{ color: textColor }}>
              {textContent}
            </span>
          </div>
        </div>
